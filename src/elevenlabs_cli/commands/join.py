...
def register(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        ...
        description="Items are file paths and 'silence:<seconds>' entries, or a spec file with 'file <path>' / 'silence <seconds>' lines. "
        "Clips are trimmed to their voiced content first (unless --no-trim), so every gap measures what the spec says.",
    )
    parser.add_argument("items", nargs="*", help="paths and silence:<seconds> entries, in order")
    ...
    parser.add_argument("--lufs", type=float, help="loudness target (default: config default_lufs; omit both for none)")
...
def run(args: Any) -> None:
    ...
    timing = audio.join(
        ...
        do_trim=args.trim, lufs=lufs,
    )
    sidecar = sidecar_path(out)
    sidecar.write_text(timing.to_json() + "\n", encoding="utf-8")
    report_saved(out)
    say(f"timing: {sidecar} ({len(timing.clips)} clips, {len(timing.gaps)} gaps, {timing.duration:.3f} s)")
    if args.verify and timing.gaps:
        check_and_report(ctx, out, [g.expected for g in timing.gaps])
